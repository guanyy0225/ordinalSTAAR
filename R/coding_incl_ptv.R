Ordinal_coding_incl_ptv <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.ptv_ds = try(OrdinalSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub.category,
                                    rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                    use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    if (is.null(dim(Geno)) | ncol(Geno) == 0) {
      ...
      result.ptv = try(OrdinalSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub.category,
                                 rare_maf_cutoff, rare_num_cutoff, combine_ultra_rare, ultra_rare_mac_cutoff,
                                 use_SPA, SPA_filter, SPA_filter_cutoff, verbose), silent = FALSE)
  ...
  OrdinalSTAAR_O_all = c("plof" = result.plof$OrdinalSTAAR_O, "plof_ds" = result.plof_ds$OrdinalSTAAR_O,
                      ...
                      "ptv" = result.ptv$OrdinalSTAAR_O, "ptv_ds" = result.ptv_ds$OrdinalSTAAR_O)
