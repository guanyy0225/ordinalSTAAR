Ordinal_plof_ds <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  # --- Part 1: Variant Selection (pLoF+Damaging Missense Definition) ---
  ...
  plof.variants <- (GENCODE.EXONIC.Category=="stopgain")|(GENCODE.EXONIC.Category=="stoploss")|(GENCODE.Category=="splicing")|(GENCODE.Category=="exonic;splicing")|(GENCODE.Category=="ncRNA_splicing")|(GENCODE.Category=="ncRNA_exonic;splicing")
  damaging.missense <- ((GENCODE.EXONIC.Category=="nonsynonymous SNV") & (MetaSVM_pred=="D"))

  plof_ds.variants <- plof.variants | damaging.missense

  variant.id.plof_ds <- variant.id.gene.current[plof_ds.variants]

  if (length(variant.id.plof_ds) < rare_num_cutoff) {
    message("Variants number of *plof_ds* is less than ", rare_num_cutoff, ", skipping...")
    return(c(list("gene_info" = gene_info_kk, "category" = "plof_ds"), list("OrdinalSTAAR_O" = NA)))
  }
  if (rm_long && length(variant.id.plof_ds) > rm_long_cutoff) {
    message(paste0("Variants number of *plof_ds* is more than ", rm_long_cutoff, ", skipping..."))
    return(c(list("gene_info" = gene_info_kk, "category" = "plof_ds"), list("OrdinalSTAAR_O" = NA)))
  }

  seqSetFilter(genofile,variant.id=variant.id.plof_ds,sample.id=phenotype.id)
