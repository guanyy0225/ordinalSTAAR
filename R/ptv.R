Ordinal_ptv <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  # --- Part 1: Variant Selection (Corrected PTV Definition) ---
  ...
  if(variant_type=="SNV") {
    ptv.variants <- ptv.snv.core | ptv.splicing
  } else if(variant_type=="Indel") {
    ptv.variants <- ptv.indel.core
  } else {
    ptv.variants <- ptv.snv.core | ptv.indel.core | ptv.splicing
  }

  variant.id.ptv <- variant.id.gene.current[ptv.variants]

  if (length(variant.id.ptv) < rare_num_cutoff) {
    message("Variants number of *ptv* is less than ", rare_num_cutoff, ", skipping...")
    return(c(list("gene_info" = gene_info_kk, "category" = "ptv"), list("OrdinalSTAAR_O" = NA)))
  }
  if (rm_long && length(variant.id.ptv) > rm_long_cutoff) {
    message(paste0("Variants number of *ptv* is more than ", rm_long_cutoff, ", skipping..."))
    return(c(list("gene_info" = gene_info_kk, "category" = "ptv"), list("OrdinalSTAAR_O" = NA)))
  }

  seqSetFilter(genofile,variant.id=variant.id.ptv,sample.id=phenotype.id)
