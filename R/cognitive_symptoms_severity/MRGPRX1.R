...
load("/mnt/project/UKB_500K_WGS_staarpipeline/Multiclass/cognitive_symptoms_severity/WGS_binary_cognitive_symptoms_severity_fullDat.20250913.Rdata")

base_covar_cols <- c("sex", "age", "age2", paste0("PC", 1:10))
# prs_pc_cols <- paste0("prs_pc", 1:5)

outcome_column <- "cognitive_symptoms_severity"
id_column <- "userId"

obj.STAAR.UKB.cognitive_symptoms_severity <- NullModel(
  phenofile = fullDat,
...
output_path <- "/UKB_500K_WGS_staarpipeline/Multiclass/cognitive_symptoms_severity/" # DNAnexus output path

message("--- Step 3: Defining Analysis Tasks ---")

analysis_tasks <- data.frame(
  chr = c(11, 11, 11, 11, 11, 11, 11),
  gene_name = c("MRGPRX1", "MRGPRX1", "MRGPRX1","MRGPRX1","MRGPRX1","MRGPRX1","MRGPRX1"),
  ...
  for (i in 1:nrow(tasks_for_this_chr)) {
    ...
    analysis_results_list_obj <- tryCatch({
      Ordinal_GeneCentricCoding(
        gene_name = gene_name,
        genofile = genofile,
        objNull = obj.STAAR.UKB.cognitive_symptoms_severity,
