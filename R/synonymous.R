Ordinal_synonymous <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  # --- Part 1: Variant Selection (Synonymous Definition) ---
  ...
  is_synonymous <- (GENCODE.EXONIC.Category=="synonymous SNV")
  variant.id.synonymous <- variant.id.gene.current[is_synonymous]

  if (length(variant.id.synonymous) < rare_num_cutoff) {
    message("Variants number of *synonymous* is less than ", rare_num_cutoff, ", skipping...")
    return(c(list("gene_info" = gene_info_kk, "category" = "synonymous"), list("OrdinalSTAAR_O" = NA)))
  }
  if (rm_long && length(variant.id.synonymous) > rm_long_cutoff) {
    message(paste0("Variants number of *synonymous* is more than ", rm_long_cutoff, ", skipping..."))
    return(c(list("gene_info" = gene_info_kk, "category" = "synonymous"), list("OrdinalSTAAR_O" = NA)))
  }

  seqSetFilter(genofile,variant.id=variant.id.synonymous,sample.id=phenotype.id)
  ...
  result.synonymous = try(OrdinalSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub,
