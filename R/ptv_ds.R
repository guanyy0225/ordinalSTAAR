Ordinal_ptv_ds <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  # --- Part 1: Variant Selection (Corrected PTV+Damaging Missense Definition) ---
  ...
  ptv.snv.core <- (GENCODE.EXONIC.Category=="stopgain")|(GENCODE.EXONIC.Category=="stoploss")
  ptv.indel.core <- (GENCODE.EXONIC.Category=="frameshift deletion")|(GENCODE.EXONIC.Category=="frameshift insertion")
  ptv.splicing <- (GENCODE.EXONIC.Category=="splicing")|(GENCODE.EXONIC.Category=="exonic;splicing")|(GENCODE.Category=="splicing")
  damaging.missense <- ((GENCODE.EXONIC.Category=="nonsynonymous SNV") & (MetaSVM_pred=="D"))

  if(variant_type=="SNV") {
    ptv_ds.variants <- ptv.snv.core | ptv.splicing | damaging.missense
  } else if(variant_type=="Indel") {
    ptv_ds.variants <- ptv.indel.core
  } else {
    ptv_ds.variants <- ptv.snv.core | ptv.indel.core | ptv.splicing | damaging.missense
  }

  variant.id.ptv_ds <- variant.id.gene.current[ptv_ds.variants]

  if (length(variant.id.ptv_ds) < rare_num_cutoff) {
    message("Variants number of *ptv_ds* is less than ", rare_num_cutoff, ", skipping...")
    return(c(list("gene_info" = gene_info_kk, "category" = "ptv_ds"), list("OrdinalSTAAR_O" = NA)))
  }
  if (rm_long && length(variant.id.ptv_ds) > rm_long_cutoff) {
    message(paste0("Variants number of *ptv_ds* is more than ", rm_long_cutoff, ", skipping..."))
    return(c(list("gene_info" = gene_info_kk, "category" = "ptv_ds"), list("OrdinalSTAAR_O" = NA)))
  }

  seqSetFilter(genofile,variant.id=variant.id.ptv_ds,sample.id=phenotype.id)
